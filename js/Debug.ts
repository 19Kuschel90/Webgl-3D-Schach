...
class VertexDebugger{
    mUniformCameraPos: any;
    mUniformPointSize: any;
    mUniformModelV: any;
    mUniformCamera: any;
    mUniformProj: any;
    mUniformColor: any;
    ...
    gl: any;
    transform: any;
	constructor(gl:any,pntSize:any){
	...
	addColor(...myarguments:any[]):this{
		if(myarguments.length == 0) return this;

        for(var i=0,c:any,p:any; i < myarguments.length; i++){
            ...
            var A:number = parseInt(c[p]	+c[p+1],16)	/ 255.0;
            var B = parseInt(c[p+2]	+c[p+3],16)	/ 255.0;
            var D =parseInt(c[p+4]	+c[p+5],16)	/ 255.0;
			this.mColor.push(
                // A,B,D
			);
		}
