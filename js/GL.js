...
function GLInstance(canvasID) {
    ...
    gl.fCreateArrayBuffer = function (floatAry, isStatic) {
        if (isStatic === undefined)
            isStatic = true; //So we can call this function without setting isStatic
    ...
    gl.fUpdateTexture = function (name, img, doYFlip, noMips) {
        ...
        if (doYFlip == true)
            this.pixelStorei(this.UNPACK_FLIP_Y_WEBGL, true); //Flip the texture by the Y Position, So 0,0 is bottom left corner.
    ...
    gl.fSetSize = function (w // width
    , h, // height 
    Unit // %, px, .....
    ) {
        if (w === void 0) { w = 600; } // width
        if (h === void 0) { h = 600; }
